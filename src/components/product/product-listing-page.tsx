...
export default function ProductListingPage({
  ...
  const [expandedSections, setExpandedSections] = useState({ price: true, size: true });

  useEffect(() => {
    const params = new URLSearchParams();
    if (gender) params.set("gender", gender);
    if (category) params.set("category", category);
    fetch(`/api/products/listing?${params}`)
      .then((r) => r.json())
  ...
  const toggleSection = (section: "price" | "size") => {
  ...
  const filteredProducts = useMemo(() => {
    ...
    return result;
  }, [products, selectedSizes, selectedPriceRanges, sortBy, priceRanges]);
      ...
      <div>
        ...
        {expandedSections.size && (
          <div className="mt-3 flex flex-wrap gap-2">
            {sizes.map((size) => (
              <button key={size} type="button" className={cn("flex h-10 items-center justify-center rounded-md border px-3 text-sm font-medium transition-colors", selectedSizes.includes(size) ? "border-zinc-900 bg-zinc-900 text-white" : "border-zinc-200 text-zinc-600 hover:border-zinc-400")} onClick={() => toggleSize(size)}>
                {size}
              </button>
            ))}
          </div>
